...
class ProdutoDAO
{
    ...
    public function __construct()
    {
        ...
        $dsn = "mysql:host=localhost;dbname=db_produtos;charset=utf8";
        $usuario = "root";
        $senha = "";
    ...
    public function save(Produto $produto) {
        try {
            if ($produto->getId() > 0) {
                // UPDATE
                $sql = "UPDATE produtos 
                        SET nome=?, preco=?, descricao=?, quantidade_estoque=?, 
                            id_categoria=?, id_marca=?, id_distribuidor=? 
                        WHERE id=?";
                ...
                $sql = "INSERT INTO produtos 
                        (nome, preco, descricao, quantidade_estoque, id_categoria, id_marca, id_distribuidor) 
                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $this->conexao->prepare($sql);
                $stmt->execute([
                    $produto->getNome(),
